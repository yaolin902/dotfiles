...
active_text_color="#3C3836"
active_bg="#FB4934"
active_bg2="#282828"
active_underline="#FB4934"
...
active_left="%{B$active_bg}%{F$active_bg2}%{F$active_text_color}"
active_right="%{F-}%{F$active_bg}"
inactive_left="%{B$inactive_bg}%{F$inactive_bg2}%{F$inactive_text_color}"
inactive_right="%{F-}%{F$inactive_bg}"
...
generate_window_list() {
	...
	while IFS="[ .\.]" read -r wid ws cname cls host title; do
		...
		if [ "${#w_name}" -gt "$char_limit" ]; then
			w_name="$(echo "$w_name" | cut -c1-$((char_limit)))"
		fi
